...
// Catches ALL unhandled JS errors and promise rejections and sends them
// to the backend so they appear in server.log and trigger the watcher.
(function setupErrorReporter() {
  function sendError(payload) {
    // fire-and-forget — don't let the reporter itself cause errors
  ...
  // Expose helper for manual error reporting from catch blocks
  window.reportClientError = function (type, message, extra = {}) {
    sendError({ type, message, url: window.location.href, ...extra });
  };
})();
...
async function loadCategories() {
  try {
    ...
    window.reportClientError("API_ERROR", `Failed to load categories: ${e.message}`);
  }
}
...
async function loadProducts() {
  try {
    ...
    window.reportClientError("API_ERROR", `Failed to load products: ${e.message}`);
  }
}
...
async function placeOrder(e) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      window.reportClientError("ORDER_ERROR", `Order failed: ${data.error}`, { status: res.status });
      return;
    }
    ...
    window.reportClientError("NETWORK_ERROR", `Checkout failed: ${err.message}`);
  }
}
...
async function loadOrders() {
  ...
  try {
    ...
    window.reportClientError("API_ERROR", `Failed to load orders: ${e.message}`);
  }
}
