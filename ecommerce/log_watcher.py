...
# ==========================================
# Bedrock client  (same pattern as DeepSeek snippet)
# ==========================================
...
def start_flask():
    ...
    if not APP_FILE.exists():
        ...
        return
    stop_flask()
    print(f"  [{ts()}] 🚀 Starting Flask (port {FLASK_PORT}) ...")
    env = os.environ.copy()
...
def restart_flask(reason: str = ""):
    global _last_size
    print(f"\n  [{ts()}] 🔄 Restarting Flask — {reason}")
    start_flask()
    time.sleep(1)
    if LOG_FILE.exists():
        _last_size = LOG_FILE.stat().st_size
        print(f"  [{ts()}] 📋 Log offset reset ({_last_size} bytes)\n")
...
def read_new_lines() -> list:
    ...
    if size == _last_size:
        return []
    if size < _last_size:
        print(f"  [{ts()}] 🔄 Log rotated — resetting offset")
        _last_size = 0
...
# ==========================================
def ask_deepseek(new_lines: list) -> dict:
    """
    ...
    try:
        ...
        # Parse — same pattern as DeepSeek snippet
...
def run_autofix():
    ...
    # autofix rewrote app.py — restart Flask with the fixed code
    restart_flask(reason="autofix rewrote app.py")

# ==========================================
# Process a batch of new log lines
# ==========================================
def process(new_lines: list, label: str = "NEW"):
...
def main():
    ...
    while True:
        try:
            ...
            if not flask_alive():
                print(f"  [{ts()}] ⚠️  Flask died — restarting ...")
                restart_flask(reason="process died")

            # Restart Flask if app.py was modified on disk
